...
if (0==ndebug || -1==ndebug) && 0~=exist(mat_file,'file')
    ...
    if 1==isempty(ix)
        return;
    elseif 1==length(ix)
        % Protect against rt=0 in init calibration: keep hardcoded rt_ref
        % if DrawISOProfile0 failed to calibrate this peptide (RT=0 sentinel).
        if 0~=AllUnHis.rt_ref(ix)
        ...
        if 1==nflag && 0~=AllUnHis.rt_ref(ix(i))
