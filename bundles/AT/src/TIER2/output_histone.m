...
% Fix D-CRIT-1 (Tribunal de Codigo 2026-05-24):
% Si la senal agregada no supera umbral, los ratios derivados son
% ruido normalizado (1.000/0.000 espurios). Emitir NaN para que el
% downstream (Python/R en AT_virgen) los trate como missing.
INTENS_THRESHOLD = 1e3;  % AUC minima para ratio defendible
total_intens = sum(auc(1:npep2,2));
if total_intens < INTENS_THRESHOLD
    auc(1:npep2,3) = NaN;
else
    for ino=1:npep2
        auc(ino,3) = auc(ino,2)/total_intens;
    end;
end;
if npep2<npep
