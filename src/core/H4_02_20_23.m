...
%%
% ========================================================================
% H4_02_20_23 — Targeted panel for histone H4 (aa 20–23) peptide "KVLR"
%                and its K20 PTMs: me1, me2, me3, ac (propionylated prep).
% ========================================================================
% This function anchors the unmodified (chemically derivatized) peptide,
% relocates RT windows for K20me1/2/3 and K20ac, extracts per-charge XICs,
% saves outputs, draws summary plots, and optionally exports PSMs.
%
% INPUTS
...
%
% OUTPUTS (to disk)
%   <cur_outpath>/H4_02_20_23.mat with:
%     His, pep_rts [5x2], pep_intens [5x2], mono_isointens [nMS1 x 5]
%   plus figures from draw_layout(...)
%   plus PSM tables if special.nDAmode == 1 (GetPSM)
%
% NOTE
%   This code keeps the exact logic/structure of your original function.
%   Only comments were added extensively for clarity.
% ========================================================================
...
% ------------------------------
unitdiff = 1.0032; % 13C spacing used by monoisotopic tracing in core helpers
...
% Prepare RT axis for plotting utilities (often named "isorts" in this codebase)
...
% Optional PSM export (historical switch)
...
% Charge states considered for this very short peptide: 1+ and 2+
His.pep_ch = repmat([1 2],length(His.mod_type),1);

% Theoretical m/z per row × charge (computed; portable across forks)
% (hard-coded table from legacy is commented in original; we compute instead)
...
% Display flags (all rows shown in figures)
His.display = ones(length(His.mod_type),1);

% Optional: reorder charge columns to put 2+ first for specific rows (3 and 4)
% This is a historical cosmetic step, not required for extraction logic
...
if main_ch~=His.pep_ch(1,1)
    ...
    % Only reorder for rows 3..4 as in the original code (tune = [3 4])
    tune = [3 4]; % 1:npep would reorder all; we replicate original behavior
    ...
    if 2~=special.nDAmode
        ...
        [His.rt_ref(1),special.ndebug] = ...
            check_ref(special.raw_path,[His.pep_seq,His.mod_type{1}], ...
                      His.rt_ref(1),special.ndebug);
        % Special fallback: if check_ref signals ndebug==2, try to infer unmod
        % position by also scanning K20me1 and aligning unmod before it.
        if 2==special.ndebug
...
if cur_rts(1)>0
    ...
    mono_isointens(:,hno)         = cur_mono_isointens;
end
...
nsplit = 1;  % pass to get_rts (splitting behavior is core-dependent)

% --- K20me1 (row 2): right of unmod (later RT)
hno = 2;
...
% Jointly resolve me2 vs me3 using a helper that considers intensity sums
