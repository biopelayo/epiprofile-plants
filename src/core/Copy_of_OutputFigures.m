...
group_colors = lines(num_groups); % Mapa de colores para los grupos
...
gscatter(score(:,1),score(:,2), sample_groups, group_colors, '.', 12);
...
h = text(min(xlim), min(ylim)*0.9, ...
    'PCA of Z-scored Log2 Ratios. Points are colored by Sample_Group. Distances reflect similarity.', ...
    'FontSize',9, 'VerticalAlignment','top');
print('-dpdf','-painters','-r600',out_file);
close();
...
figure('visible','off');
setup_publication_figure();
set(gcf,'Units','inches','Position',[0 0 10 6]); % Más ancho para grupos
hold on;
...
box on;
% Añadir líneas de grupo (cada grupo tiene 2 barras, H3 y H4)
for b = 1:length(group_boundaries)
