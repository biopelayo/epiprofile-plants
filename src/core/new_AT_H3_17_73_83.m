function H3_17_73_83(MS1_index,MS1_peaks,MS2_index,MS2_peaks,ptol,cur_outpath,special)
%%
% ============================== OVERVIEW (EN) ================================
% Targeted quantification for a panel of two H3 peptides (see init_histone),
% across charge states defined in His.pep_ch. The function:
%   1) Skips computation if an output .mat already exists (idempotent run).
%   2) Initializes the peptide panel (labels, mods, charge states, m/z, RT anchors).
%   3) Computes representative retention times (RTs), integrates intensities,
%      and builds monoisotopic traces from MS1 (with optional MS2-assisted steps).
%   4) Writes results to disk and draws diagnostic figures.
...
%   MS1_peaks   : MS1 peaks container/struct (format expected by downstream functions).
...
%   - <cur_outpath>/H3_17_73_83.mat via output_histone().
%   - Figures via draw_layout().
%   - Optional PSM evidence via GetPSM2() if special.nDAmode==1.

% ----------------------------------- check -----------------------------------
out_filename = 'H3_17_73_83';                                  % fixed stem used for outputs
...
if 0~=exist(out_file0,'file')                                   % if the .mat already exists...
...
His = init_histone();                                           % define peptide panel:
                                                                ...
                                                                %  - rt_ref (reference RT anchors), display flags

% --------------------------------- calculate ---------------------------------
unitdiff = 1.0032;                                              % ~C13-C12 isotopic mass difference (Da)
[pep_rts,pep_intens,mono_isointens] = ...
    calculate_layout(MS1_index,MS1_peaks,MS2_index,MS2_peaks, ...% core quantification routine:
                     ptol,unitdiff,His,special);                %  - RT detection/integration
                                                                %  - monoisotopic trace extraction
...
His.mod_short = {'EIAQDFKTDLR';                                 % two peptide short labels
    'EIAQDYKTDLR'};
His.mod_type = {'0,pr;7,pr;';                                   % modification encodings per peptide:
    '0,pr;7,pr;'};                                              %  indices are peptide coords (0 often N-term)
                                                                %  'pr' typically denotes propionylation (see README)

His.pep_ch = repmat([1 2 3 4],length(His.mod_type),1);          % consider charges z = 1, 2, 3, 4 for all peptides
%{
His.pep_mz = [900.5302  450.7687  300.8482                      % OPTIONAL example of hard-coded m/z values
...
[npep,ncharge] = size(His.pep_mz);                               % dimensions taken from the m/z grid
...
if 1~=special.ndebug                                             % allow RT recalibration unless in debug
    if 2~=special.nDAmode                                        % standard confirmation (nDAmode ≠ 2)
        ...
        special.ndebug = 1;                                      % lock anchors after DA refinement (freeze changes)
    end;
end;
...
for hno=1:2                                                      % iterate over each targeted peptide
    ...
    if cur_rts(1)>0                                             % guard: only keep if RT is valid
        pep_rts(hno,1:ncharge) = cur_rts;                       % store RTs for all defined charges
