function HH2A_AT03_Z_33_41(MS1_index,MS1_peaks,MS2_index,MS2_peaks,ptol,cur_outpath,special)
%%
% H2A.Z.11+Z.8 peptide AGIQFPVGR (residues 33-41)
% No K residues -> only unmod form (N-terminal propionylation)
...
His.pep_seq = 'AGIQFPVGR';
His.mod_short = {'unmod'};
His.mod_type = {'0,pr;'};

His.pep_ch = repmat([1 2 3],length(His.mod_type),1);
His.pep_mz = calculate_pepmz(His);
His.rt_ref = [33.12];% unmod RT from 0_ref_info.mat
...
% calibrate the rt_ref (only unmod, no other forms to shift)
if cur_rts(1)>0
