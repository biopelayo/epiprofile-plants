...
for i=1:length(raw_names)
    ...
    if 0==exist(ms1_file,'file')
        ...
        % RawToMS1 may generate .MS1 in pwd or in raw_path — check both
        ms1_in_pwd = fullfile(pwd,[raw_names{i},'.MS1']);
        ms1_in_raw = fullfile(raw_path,[raw_names{i},'.MS1']);
        if 0~=exist(ms1_in_raw,'file')
            movefile(ms1_in_raw,ms1_path);
        elseif 0~=exist(ms1_in_pwd,'file')
            movefile(ms1_in_pwd,ms1_path);
        else
            fprintf(1,'warning: MS1 file not found after conversion: %s\n',raw_names{i});
        end;
    end;
