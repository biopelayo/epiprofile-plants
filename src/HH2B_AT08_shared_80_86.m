function HH2B_AT08_shared_80_86(MS1_index,MS1_peaks,MS2_index,MS2_peaks,ptol,cur_outpath,special)
%%
% Adapted from HH2B_01m1B_80_86.m (human LAHYNKR)
% AT H2B all 9 variants: LARYNKK (H->R at pos3, R->K at pos7)
% Two substitutions vs human: both biologically significant
%   - H->R: loses imidazole, gains guanidinium (charge change)
%   - R->K at C-term: terminal K needs propionylation; not Arg-C endpoint
% Preceded by R in 2/9 variants (Q9LFF6, Q9SF55) = true Arg-C
% Preceded by K in 7/9 variants = requires missed cleavage or partial digest
% Conserved in ALL 9 AT H2B isoforms (100% conservation)
...
%%
% AT H2B: LARYNKK
% vs human: LAHYNKR (H->R at pos3, R->K at pos7)
% K6 and K7 both need propionylation in unmod state
% Y4 is the modifiable tyrosine (equivalent to human Y83)

His.pep_seq = 'LARYNKK';
His.mod_short = {'unmod';
    'Yac';
    'Ypr'};
His.mod_type = {'0,pr;6,pr;7,pr;';
    '0,pr;4,ac;6,pr;7,pr;';
    '0,pr;4,pr;6,pr;7,pr;'};
